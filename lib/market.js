...
// Блокліст — стейблкоїни, токенізовані активи, непридатні для трейдингу
const BLOCKLIST = [
  // Токенізовані метали/активи
  'PAXG-USDT',   // токенізоване золото (PAX Gold)
  'XAUT-USDT',   // токенізоване золото (Tether Gold) ✅ новий
  'WBTC-USDT',   // wrapped bitcoin
  'WETH-USDT',   // wrapped ethereum ✅ новий
  'HBTC-USDT',   // wrapped bitcoin ✅ новий
  // Стейблкоїни
  'USDC-USDT',
  'DAI-USDT',
  'TUSD-USDT',
  ...
  try {
    ...
    const sorted = data.data
      .filter(t =>
        ...
        !BLOCKLIST.includes(t.instId) &&        // ✅ блокліст
        parseFloat(t.volCcy24h) > 500000 &&     // ✅ мінімум $500K об'єму за 24г
        parseFloat(t.last) > 0.001              // ✅ фільтр мікромонет (KAT, SHIB тощо)
      )
    ...
    if (!sorted.length) {
      console.warn('⚠️ Порожній список після фільтрів — використовуємо запасний');
      return getFallback(limit);
    }

    console.log('📊 Топ ' + sorted.length + ' монет по об\'єму:', sorted.join(', '));
    return sorted;
