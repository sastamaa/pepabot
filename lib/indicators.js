...
function analyzeSignal(klines, symbol) {
  ...
  // 🟢 LONG: EMA вгору + RSI норм + (WT умова АБО Demand зона)
  if (
    ema20 > ema50 &&
    rsiVal > 45 && rsiVal < 75 &&
    ...
    const score = rsiVal
                ...
                + demandBonus + wtOversoldBonus;  // ✅ нові бонуси
    return {
      direction:   'LONG',
      price,       score,
      ...
      demand:      ds.demand   ? ('zone ' + ds.demand.bottom.toFixed(4) + '-' + ds.demand.top.toFixed(4) + (ds.demand.priceInZone ? ' 🎯' : '')) : null,  // ✅ нове
      sl:          (price - atrVal * 2).toFixed(4),
      tp:          (price + atrVal * 4).toFixed(4)
    };
  }

  // 🔴 SHORT: EMA вниз + RSI норм + (WT умова АБО Supply зона)
  if (
    ema20 < ema50 &&
  rsiVal > 45 && rsiVal < 70 &&          // ← БУЛО: rsiVal < 55 && rsiVal > 20
    ...
    const score = (100 - rsiVal)
                ...
                + supplyBonus + wtOverboughtBonus;  // ✅ нові бонуси
    return {
      direction:   'SHORT',
      price,       score,
      rsi:         rsiVal.toFixed(1),
      ...
      supply:      ds.supply   ? ('zone ' + ds.supply.bottom.toFixed(4) + '-' + ds.supply.top.toFixed(4) + (ds.supply.priceInZone ? ' 🎯' : '')) : null,  // ✅ нове
      sl:          (price + atrVal * 2).toFixed(4),
      tp:          (price - atrVal * 4).toFixed(4)
    };
  }
