...
function analyzeSignal(klines, symbol) {
  ...
  // ✅ КОНТРТРЕНД: дозволяємо LONG проти тренду якщо 3 умови виконані
  const strongReversal =
    ds.demand?.priceInZone &&   // ціна в Demand зоні
    rsiVal < 40 &&              // перепроданість
    wtOversold &&               // WT розвернувся з < -50
    priceChange > -1.5;         // не в активному падінні
  ...
  if (
    ...
    const score = rsiVal
      ...
      // В indicators.js — в кінці перед return, для обох LONG і SHORT:
  const risk   = Math.abs(price - parseFloat((price - atrVal * 2).toFixed(4)));
  const reward = Math.abs(price - parseFloat((price + atrVal * 4).toFixed(4)));
  if (reward / risk < 1.5) return null;  // ✅ фільтр поганих угод
    ...
    return {
      ...
      contraTrend: isContraTrend,   // ✅ нове — попередження в Telegram
