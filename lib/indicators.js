...
function analyzeSignal(klines, symbol = '') {
  ...
  // ATR фільтр — пом'якшений (було 0.15, тепер 0.05)
  if (atrPercent < 0.05) {
    console.log(`${symbol} — пропущено: занадто тихий (ATR ${atrPercent.toFixed(3)}%)`);
    return null;
  }
  ...
  // Підтвердження — достатньо 2 з 3 свічок (було всі 3)
  const last3 = closes.slice(-3);
  const upCount   = (last3[2]>last3[1] ? 1:0) + (last3[1]>last3[0] ? 1:0);
  const downCount = (last3[2]<last3[1] ? 1:0) + (last3[1]<last3[0] ? 1:0);
  const confirmedUp   = upCount >= 1;   // ← було 2/2, тепер достатньо 1/2
  const confirmedDown = downCount >= 1;
  ...
  if (
    ema20 > ema50 &&
    rsiVal > 50 && rsiVal < 78 &&   // ← було 52/75
    volRatio > 1.0 &&               // ← було 1.1
    confirmedUp &&
    priceChange > 0.1               // ← було 0.2
  ) {
  ...
  if (
    ema20 < ema50 &&
    rsiVal < 50 && rsiVal > 22 &&   // ← було 48/25
    volRatio > 1.0 &&
    confirmedDown &&
    priceChange < -0.1              // ← було -0.2
  ) {
