...
function analyzeSignal(klines, symbol = '') {
  ...
  console.log(`${symbol} | RSI:${rsiVal.toFixed(1)} | vol:${volRatio.toFixed(2)}x | change:${priceChange.toFixed(2)}%`);

  // 🟢 LONG
  if (ema20 > ema50 && rsiVal > 45) {
    // Score: чим вищий RSI і об'єм — тим краще
    const score = rsiVal + (volRatio * 10) + (priceChange * 5);
    return {
      direction: 'LONG',
      price, score,
  ...
  if (ema20 < ema50 && rsiVal < 55) {
    const score = (100 - rsiVal) + (volRatio * 10) + (Math.abs(priceChange) * 5);
    return {
      direction: 'SHORT',
      price, score,
      rsi: rsiVal.toFixed(1),
