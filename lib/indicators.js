  ...
  for (let i = klines.length - 1; i >= 2 && i >= klines.length - 20; i--) {
    ...
    if (!result.bullish && prev2.high < curr.low) {
      ...
      if (price > gapTop && gapSize > 0.05) {
        result.bullish = { top: gapTop.toFixed(4), bottom: gapBottom.toFixed(4), size: gapSize.toFixed(3) };
      }
    }
    if (!result.bearish && prev2.low > curr.high) {
      ...
      if (price < gapBottom && gapSize > 0.05) {
        result.bearish = { top: gapTop.toFixed(4), bottom: gapBottom.toFixed(4), size: gapSize.toFixed(3) };
      }
    }
  }
  return result;
...
function detectDemandSupply(klines) {
  ...
  for (let i = 2; i < len - 1; i++) {
    ...
    // В функції detectDemandSupply заміни demandZone:
if (!demandZone && c.close > c.open && body > range * 0.6 &&
    prev1.close < prev1.open && prev2.close < prev2.open) {
  const zoneBottom = Math.min(c.open, c.close);
  const zoneTop    = Math.max(c.open, c.close);

  // ✅ Demand має бути НИЖЧЕ поточної ціни
  if (zoneTop < price) {
    demandZone = {
      bottom:      zoneBottom,
      top:         zoneTop,
      priceInZone: price >= zoneBottom && price <= zoneTop * 1.005
    };
  }
}

// І Supply має бути ВИЩЕ ціни:
if (!supplyZone && c.close < c.open && body > range * 0.6 &&
    prev1.close > prev1.open && prev2.close > prev2.open) {
  const zoneBottom = Math.min(c.open, c.close);
  const zoneTop    = Math.max(c.open, c.close);

  // ✅ Supply має бути ВИЩЕ поточної ціни
  if (zoneBottom > price) {
    supplyZone = {
      bottom:      zoneBottom,
      top:         zoneTop,
      priceInZone: price >= zoneBottom * 0.995 && price <= zoneTop
    };
  }
}
...
function analyzeSignal(klines, symbol) {
  ...
  const ds     = detectDemandSupply(klines);
  ...
  const wtOversold    = wt && wtVal < -50 && wt.crossUp;
  const wtOverbought  = wt && wtVal > 50  && wt.crossDown;
  ...
  const demandBonus       = ds.demand?.priceInZone ? 30 : 0;
  const supplyBonus       = ds.supply?.priceInZone ? 30 : 0;
  const wtOversoldBonus   = wtOversold             ? 25 : 0;
  const wtOverboughtBonus = wtOverbought           ? 25 : 0;
  ...
  const longConfirmations = [
    ...
    wtOversold,
    ds.demand?.priceInZone,
    ob.bullish?.priceInZone,
    fvg.bullish !== null
  ].filter(Boolean).length;

  if (
    (ema20 > ema50 || strongReversal) &&
    rsiVal > 35 && rsiVal < 65 &&
    priceChange > -1.5 &&
    longConfirmations >= 3                           // ✅ мінімум 2 підтвердження
  ) {
    const slPrice = price - atrVal * 2;
    const tpPrice = price + atrVal * 4;
    if (Math.abs(price - tpPrice) / Math.abs(price - slPrice) < 1.5) return null; // ✅ R:R фільтр

    const isContraTrend = ema20 <= ema50;
    const score = rsiVal + (volRatio * 5) + (atrPercent * 20)
                + (wt && wt.crossUp ? 20 : 0)
                + fvgBonusLong + obBonusLong + demandBonus + wtOversoldBonus;
    return {
      direction:   'LONG',
      price,       score,
  ...
  const shortConfirmations = [
    wtShort && downCount >= 1,
    wtOverbought,
    ds.supply?.priceInZone,
    ob.bearish?.priceInZone,
    fvg.bearish !== null
  ].filter(Boolean).length;
  ...
  if (
    ...
    rsiVal > 45 && rsiVal < 65 &&
    priceChange < 1.0 &&
    shortConfirmations >= 3                          // ✅ мінімум 2 підтвердження
  ) {
    const slPrice = price + atrVal * 2;
    const tpPrice = price - atrVal * 4;
    if (Math.abs(price - tpPrice) / Math.abs(price - slPrice) < 1.5) return null; // ✅ R:R фільтр правильний для SHORT

    const score = (100 - rsiVal) + (volRatio * 5) + (atrPercent * 20)
                + (wt && wt.crossDown ? 20 : 0)
                + fvgBonusShort + obBonusShort + supplyBonus + wtOverboughtBonus;
    return {
      direction:   'SHORT',
      price,       score,
      rsi:         rsiVal.toFixed(1),
